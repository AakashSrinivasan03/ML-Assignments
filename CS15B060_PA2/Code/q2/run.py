...
LDA_analysis=LinearDiscriminantAnalysis()###performing Linear Discriminant analysis:Note only one direction is desired
LDA_analysis.fit(training,y_train)
X_train=LDA_analysis.transform(training) ##Projecting train data in this direction
direction=np.append(np.array(LDA_analysis.intercept_),LDA_analysis.coef_)
#direction=Princ_comp_analysis.components_;
#print y_train.shape
train_dataset=np.append(X_train,y_train.reshape(y_train.shape[0],1),1)
X_test=LDA_analysis.transform(testing); ##Projecting test data in this direction
#print "hello"
#print X_test.shape
...
accuracy=sklearn.metrics.accuracy_score(y_test, testing_prediction_y) #######Precision,recall,F1-score
...
pyplot.scatter(X_train[1000:2000,:],y_train[1000:2000,:],c='b')   ##################Performing linear regression on projected datasets
...
pyplot.scatter(X_train[:1000,:],np.zeros(1000),c='r',marker='x')  ################Showing classifier boundry
